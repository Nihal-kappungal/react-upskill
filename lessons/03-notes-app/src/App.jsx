    ...
    <div className='flex items-center justify-between h-dvh w-full p-10'>
      ...
      <div className='flex flex-wrap gap-4  w-1/2 h-full '>
        {notes.map(function(note,idx){
          return <div 
                  key={idx}
                  className='bg-gray-100 w-50 h-40 border rounded-2xl p-4 '>
            <h4 className='text-2xl font-bold leading-tight '>{idx} {note.title}</h4>
            <hr />
            <p className='mt-2.5 text-sm text-gray-500 '>{note.details}</p>
            <button onClick={()=>{
              deleteNote(idx);
            }} className='mt-6 bg-red-500 rounded px-4 py-1.5 w-full text-white font-bold'>delete</button>
        </div>
        })}
      </div>
    </div>
